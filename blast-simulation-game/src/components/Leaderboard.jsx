...
const LeaderboardModal = ({ show, onClose }) => {
  ...
  const uniquePlayers = useMemo(() => {
    const names = allScores
      .map((s) => s.playerName || "Anonymous")
      .filter(Boolean);
    ...
    <div className="fixed inset-0 h-full bg-black/60 backdrop-blur-sm flex justify-center items-center z-[1000] p-4 animate-fadeIn">
      <div className="bg-gray-900 border border-white/20 text-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="p-3 bg-white/5 border-b border-white/10 flex flex-col sm:flex-row gap-3">
          ...
          {/* Time Buttons */}
          <div className="flex justify-between sm:justify-start bg-black/30 rounded-lg p-1 border border-white/10">
          ...
          {filteredScores.length === 0 ? (
            ...
            <>
              ...
              <div className="sm:hidden">
                {/* Header Row */}
                <div className="bg-white/5 sticky top-0 backdrop-blur-md z-10 p-3 flex justify-between items-center border-b border-white/10">
                  <div className="text-xs font-bold uppercase text-white/50 text-center w-12">
                    #
                  </div>
                  <div className="text-xs font-bold uppercase text-white/50 flex-1 text-center">
                    Player
                  </div>
                  <div className="text-xs font-bold uppercase text-white/50 text-right w-20">
                    Score
                  </div>
                </div>
                ...
                <div className="divide-y divide-white/5">
                  {filteredScores.map((entry, index) => {
                    ...
                    if (isGold) {
                      rankClass =
                        "text-yellow-400 font-black text-lg drop-shadow-sm";
                      rowClass = "bg-yellow-500/5 hover:bg-yellow-500/10";
                      icon = (
                        <Trophy className="w-3 h-3 text-yellow-500 ml-1 inline" />
                      );
                    } else if (isSilver) {
                    ...
                    return (
                      <div
                        key={index}
                        className={`${rowClass} transition-colors p-3 flex justify-between items-center`}
                      >
                        <div className={`text-center w-12 ${rankClass}`}>
                          {index + 1}
                          {icon}
                        </div>
                        <div className="flex-1 text-center">
                          <span
                            className={`font-medium ${
                              isGold ? "text-yellow-100" : "text-white/90"
                            }`}
                          >
                        ...
                        <div
                          className={`text-right font-mono text-base font-bold w-20 ${
                            isGold ? "text-yellow-400" : "text-green-400"
                          }`}
                        >
              ...
              <table className="hidden sm:table w-full text-left border-collapse">
                <thead className="bg-white/5 sticky top-0 backdrop-blur-md z-10">
                  <tr>
                    <th className="p-3 text-xs font-bold uppercase text-white/50 w-14 text-center">
                      #
                    </th>
                    <th className="p-3 text-xs font-bold uppercase text-white/50">
                      Player
                    </th>
                    <th className="p-3 text-xs font-bold uppercase text-white/50 text-right">
                      Score
                    </th>
                    <th className="p-3 text-xs font-bold uppercase text-white/50 text-right">
                      Efficiency
                  ...
                  {filteredScores.map((entry, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td
                          className={`p-3 text-right font-mono text-base font-bold ${
                            isGold ? "text-yellow-400" : "text-green-400"
                          }`}
                        >
                          {entry.score.toLocaleString()}
                        ...
                        <td className="p-3 text-right text-xs text-white/40 font-mono">
        ...
        <div className="p-3 border-t border-white/10 bg-black/20 flex justify-between items-center">
          <div className="text-xs text-white/30">
            Top {filteredScores.length} results
          </div>
          {!showConfirmReset ? (
            <button
